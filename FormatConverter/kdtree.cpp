...
// utility
  ...
  inline long double squared(const long double x) {
    return(x*x);
  }
  ...
  //
  long double KDTreeResultVector::max_value() {
  ...
  long double KDTreeResultVector::replace_maxpri_elt_return_new_maxpri(KDTreeResult& e) {
  ...
  KDTreeNode* KDTree::build_tree_for_range(int l, int u, KDTreeNode* parent) {
    ...
    if ((u-l) <= bucketsize) {
      ...
      for (int i=0;i<dim;i++) {
        ...
        long double spread = node->box[i].upper - node->box[i].lower;
        if (spread>maxspread) {
          maxspread = spread;
          c=i;
        }
      ...
      if (false) {
        ...
        if (true) {
          ...
          average = sum / static_cast<long double> (u-l+1);
        } else {
  ...
  void KDTree::spread_in_coordinate(int c, int l, int u, interval& interv) {
    ...
    long double smin, smax;
    long double lmin, lmax;
    int i;
    ...
    if (i == u+1) {
      long double last = the_data[ind[u]] [c];
  ...
  int KDTree::select_on_coordinate_value(int c, long double alpha, int l, int u) {
    //
  ...
  static const long double infinity = 1.0e38;
  ...
  private:
    ...
    std::vector<long double>& qv;
    int dim;
    bool rearrange;
    unsigned int nn; // , nfound;
    long double ballsize;
  ...
  public:
    SearchRecord(std::vector<long double>& qv_in, KDTree& tree_in,
  ...
  void KDTree::n_nearest_brute_force(std::vector<long double>& qv, int nn, KDTreeResultVector& result) {
  ...
  void KDTree::n_nearest(std::vector<long double>& qv, int nn, KDTreeResultVector& result) {
    SearchRecord sr(qv, *this, result);
    std::vector<long double> vdiff(dim, 0.0);
  ...
  void KDTree::n_nearest_around_point(int idxin, int correltime, int nn,
    ...
    std::vector<long double> qv(dim); //  query vector
    result.clear();
    for (int i=0; i<dim; i++) {
  ...
  void KDTree::r_nearest(std::vector<long double>& qv, long double r2, KDTreeResultVector& result) {
  ...
  int KDTree::r_count(std::vector<long double>& qv, long double r2) {
  // search for all within a ball of a certain radius
    {
      KDTreeResultVector result;
      SearchRecord sr(qv, *this, result);
  ...
  void KDTree::r_nearest_around_point(int idxin, int correltime, long double r2,
          KDTreeResultVector& result) {
    std::vector<long double> qv(dim);  //  query vector
  ...
  int KDTree::r_count_around_point(int idxin, int correltime, long double r2) {
    std::vector<long double> qv(dim);  //  query vector
  ...
  void KDTreeNode::search(SearchRecord& sr) {
    ...
    if ( (left == NULL) && (right == NULL)) {
      ...
      long double extra;
      long double qval = sr.qv[cut_dim];
  ...
  inline long double dis_from_bnd(long double x, long double amin, long double amax) {
    if (x > amax) {
      return(x-amax);
    } else if (x < amin)
      return (amin-x);
  ...
  inline bool KDTreeNode::box_in_search_range(SearchRecord& sr) {
    ...
    int dim = sr.dim;
    long double dis2 =0.0;
    long double ballsize = sr.ballsize;
    for (int i=0; i<dim;i++) {
