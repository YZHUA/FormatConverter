...
namespace kdtree {
  ...
  typedef boost::multi_array<long double, 2>           KDTreeArray;
  typedef boost::const_multi_array_ref<long double, 2> KDTreeROArray;
  ...
  struct KDTreeResult {
  public:
    long double dis;  // square distance
  ...
  public:
    ...
    long double replace_maxpri_elt_return_new_maxpri(KDTreeResult&);
  ...
  public:
    ...
    void n_nearest_brute_force(std::vector<long double>& qv, int nn, KDTreeResultVector& result);
    // search for n nearest to a given query vector 'qv' usin
    // exhaustive slow search.  For debugging, usually.
    
    void n_nearest(std::vector<long double>& qv, int nn, KDTreeResultVector& result);
    ...
    void r_nearest(std::vector<long double>& qv, long double r2, KDTreeResultVector& result);
    // search for all neighbors in ball of size (square Euclidean distance)
    // r2. Return number of neighbors in 'result.size()',
    
    void r_nearest_around_point(int idxin, int correltime, long double r2,
            KDTreeResultVector& result);
    // like 'r_nearest', but around existing point, with decorrelation
    // interval.
    
    int r_count(std::vector<long double>& qv, long double r2);
    // count number of neighbors within square distance r2.
    
    int r_count_around_point(int idxin, int correltime, long double r2);
    // like r_count, c
  ...
  private:
    ...
    int select_on_coordinate_value(int c, long double alpha, int l, int u);
  ...
  private:
    ...
    long double cut_val, cut_val_left, cut_val_right;  //cut value
